...
export default function ExportOptions({ intentId }: { intentId?: string }) {
    ...
    if (format === "webhook") {
      try {
        const res = await fetch("/api/export", {
          ...
          body: JSON.stringify({ url: "https://example.com/webhook", payload: { intentId } })
        });
       ...
       setTimeout(() => setStatus("Exported " + format + " successfully"), 1000);
    }
  };
  ...
  return (
    <div className="bg-white border rounded-xl p-6 shadow-sm">
      ...
      {status && <p className="mt-4 text-sm text-emerald-600 text-center font-medium">{status}</p>}
    </div>
  );
}
