...
- **An agent with a spine, not an LLM with a sidebar.** The Studio runs a *perceive → decide → act* loop with exactly one job: drive the record to Ready. Each turn it reads the record's gaps and makes the single highest-value move — infer, ask, disambiguate, surface a conflict, recommend from a similar past intent, or split two objectives apart — holds the Quality Gate line, and *knows when to stop*. It reasons over prior intents **and their outcomes**, so a request that mirrors three past failures is met with the lesson, not a blank prompt. See the companion design doc §3.4–3.7 for the behavioral spec.
