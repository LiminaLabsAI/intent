...
| 12 | Working-Memory Build Flow & Cost Engine | pick mode → **clarify** (ask gaps + **verify inferred assumptions**) with the panel empty & only 🔴/🟡/🟢 live → **build gate** → **build run** materializes working memory + graph + **actual cost vs estimate**. Ratifies the config-as-data cost engine (ADR-0001) + persona picker. | refines 1–5 + advisory (+ actual) | ⏳ **next** |
| 13 | Precedent & Memory | pgvector semantic memory over prior intents **+ outcomes** · recommend-from-precedent / offer-reuse moves · confidence floor + cold-start · emergent-slot promotion loop | Continuous Feedback (near-term) | ⏳ |
| 14 | Governance Engine (Approval Decision) | stage-6 engine: evaluate Evidence Quality · Policy & Compliance · Semantic Conflicts · Risk & Impact · Delegation (EACI) · Autonomy Eligibility → **5 outcomes** (Auto-Approved · Needs-Clarification · Human-Review · Conditional · Rejected) · drive governance lifecycle states · **enforce the invariant** (rule-based first) | **6 — the missing half** | ⏳ |
| 15 | Human Review Workflow | owner/SME routing map · review tasks · SLA · Review&Evaluate → Decision (approve / request-changes / reject) → Provide Reason (comments · conditions · constraints) · conditional-approval monitor | Human Review · Rejected · Conditional | ⏳ |
| 16 | Handoff & Dispatch | working-memory export (OKF/MD + compression **projection**, never a mutation, §5.1) · artifact expansion (PRD/Plan) · dispatch to a downstream executor with the **persona/config attached** · INT-XXXXX minted at approval | 7 (ID at approval) · handoff | ⏳ |
| 17 | Normalization & Ontology | stage-4: ontology mappings · synonym resolution · standardized intent · domain alignment (feeds the gate's "Valid Domain" check) | **4 — genuinely absent** | ⏳ |
| 18 | Multi-source Capture | Applications/APIs · Agents/Automation · Documents/Emails → the same canonical record (headless capture writes to the trunk) | 1 (other sources) | ⏳ |
| 19 | Progressive Autonomy & Flywheel | outcome capture → patterns & insights → quality & process → models & policies → **widen the auto-approve envelope** | Progressive Autonomy | ⏳ (needs volume) |
| 20 | **V2** — Attached-System Execution Engine | new-vs-existing-system fork · attach harness (repo/dirs/docs) · the senior's full engine: **`budget_ratio`** context allocation · **RAG memory-strategy** (In-Context / Naive / Graph-RAG, chosen by dataset×context) · **overflow→RAG** fallback · **model tiers** + live pricing · **provider payload** (OpenAI/Anthropic/Gemini) · **real execution cost** · **budget auto-pick** ($X→best mode) | execution runtime (ADR-0002 §3) | ⏳ (V2) |
| 21 | **Future** — Execution Agent in Flow | an agent that consumes the built working memory and **runs it** inside Flow; generic outcome (PRD / user story / doc / code); per-user precedent memory folds into Phase 13 | execution (ADR-0002 §3) | 🔭 future |

> **Note (2026-07-15):** Phase 12 inserted; the governance/capture arc renumbered 12–18 → 13–19. Phases 20–21 capture the deferred **V2 / Future** scope from [ADR-0002](../decisions/0002-working-memory-build-flow-and-cost.md) so nothing is lost before it becomes a full spec.
